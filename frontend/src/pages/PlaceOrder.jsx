...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      switch (method) {
        //API call for Cash on Delivery orders
        case 'cod': {
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        ...
        <div className='flex gap-3'>
          ...
          <input required onChange={onChangeHandler} name='county' value={formData.county} className='border border-gray-500 rounded py-1.5 px-3.5 w-full' type="text" placeholder='County' />
        </div>
        <div className='flex gap-3'>
          <input required onChange={onChangeHandler} name='postcode' value={formData.postcode} className='border border-gray-500 rounded py-1.5 px-3.5 w-full' type="text" placeholder='Postcode' />
